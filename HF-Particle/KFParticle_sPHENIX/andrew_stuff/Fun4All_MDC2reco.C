...
void Fun4All_MDC2reco(int n = 0, vector<string> myInputLists = {"condorJob/fileLists/productionFiles-CHARM-dst_tracks-00000.list"}, const int nEvents = 10000)
{
  ...
  string fileNumber = std::to_string(n);
  ...
  //infile->AddListFile("ups2.txt");
  for(int i=0; i<100; i++) {
    //if(i == 27 || i == 78 || i == 82){++i;}
    sprintf(infilename,"/sphenix/sim/sim01/sphnxpro/MDC1/embed/new/pythia8_DST_Upsilon_%d.root",100 + i);

    //    cout << "in:  " << infilename << endl;
    infile->AddFile(infilename);
    }
  //sprintf(infilename,"/sphenix/sim/sim01/sphnxpro/MDC1/embed/new/pythia8_DST_Upsilon_%d.root",100 + 27);
  //infile->AddFile(infilename);
  se->registerInputManager(infile);
  ...
  if (runTruthTrigger)
  {
    ...
    myFinder->setEtaRange(-5, 5); //Note: sPHENIX acceptance is |eta| <= 1.1
  ...
  /*auto f = new TFile("/gpfs/mnt/gpfs02/sphenix/user/aclarke/analysis/HF-Particle/KFParticle_sPHENIX/hists.root", "UPDATE"); 
  f->WriteObject(vtx, "vtx"); f->Close();
  f->Close()*/
